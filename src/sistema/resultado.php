...
<META HTTP-EQUIV="REFRESH" CONTENT="15;URL=resultado.php?prueba=<?php echo $prueba ?>"> 
...
//print_r($cuenta_cat);
 ...
 $sql_record = "SELECT MIN(tiempo_RI) as minimo FROM concurso WHERE Id_prueba=".$prueba." AND faltas_RI = 0 AND tiempo_RI <> 0 ";
 
 //AND faltas_2R = 0 AND tiempo_2R <> 0
...
$filas_resultado = mysqli_num_rows ($resultados);
      ...
      $sql_listado = "select * from concurso WHERE Id_prueba=".$prueba." AND finaliza=0 ORDER BY CAST(orden_ingreso AS unsigned) ASC";
...
<?PHP
	...
	if ($colorfila==0){ 
         ...
         print ("No hay mas participantes");
