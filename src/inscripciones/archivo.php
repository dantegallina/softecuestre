...
if ( $term = $_GET['term'] ?? '' ) {
   $sql="SELECT * FROM `feajinetes` WHERE `nombre` LIKE '".$term."' AND `disciplina` LIKE 'Salto Jinetes'";
  ...
  $matches = array();
   ...
   while($row = mysqli_fetch_array($evento1)){
    $date = date_create($row['nacimiento']);
      array_push($matches, $row['id'], $row['nombre'], $row['licencia'], $row['categoria'], $row['club'], date_format($date,"d/m/Y") );
}
